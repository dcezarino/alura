...
import br.com.alura.screenmatch.principal.model.*;
import br.com.alura.screenmatch.principal.repository.SerieRepository;
...
public class Principal {
    ...
    private final String API_KEY = "&apikey=8be27f04";
        ...
        while (opcao != 0) {
            ...
            switch (opcao) {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    buscarSerieTotalTemporadasEAvaliacao();
                    break;
                case 9:
    ...
    private void buscarSerieWeb() {
        ...
        // Verifica se a série já está cadastrada com base no título
        Optional<Serie> serieExistente = serieRepository.findByTitulo(serie.getTitulo());

        if (serieExistente.isPresent()) {
            System.out.println("Série: " + serie.getTitulo() + " já cadastrada: " + serieExistente.get());
        } else {
    ...
    private void buscarEpisodioPorSerie() {
        ...
        if (serie.isPresent()) {
            ...
            serieRepository.save(serieEncotrada);
    ...
    private void buscarSeriesPorCategoria() {
        ...
        List<Serie> seriesPorCategoria = serieRepository.findByGenero(Categoria.fromPtBr(nomeGenero));
    ...
    private void buscarSerieTotalTemporadasEAvaliacao() {

        System.out.println("Deseja buscar séries de até quantas temporadas? ");
        var totalTemporadas = leitura.next();

        System.out.println("Avaliações a partir de que valor? ");
        var avaliacao = leitura.nextDouble();

        List<Serie> seriesPorTemporadas = serieRepository.seriesPorTemporadaEAvaliacao(totalTemporadas, avaliacao);

        System.out.println("Séries por total temporadas " + totalTemporadas + " e " + "avaliação " + avaliacao);
    ...
    private void buscarEpisodioPorTrecho() {
        ...
        System.out.println("Informe a descrição do episódio para efetuar a buscar. ");
        var trechoEpisodio = leitura.nextLine();

        List<Episodio> episodiosPorTrecho = serieRepository.episodioPorTrecho(trechoEpisodio);
